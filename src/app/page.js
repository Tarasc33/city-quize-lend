...
import {ref, set, get, onValue, child, update} from "firebase/database"
...
import CloudWish from "@/components/CloudWish/CloudWish"
...
const Home = () => {
  ...
  const submit = () => {
    const id = uuid()
    const fontType = ["normal", "italic"]
    const randFontType = fontType[Math.floor(Math.random() * fontType.length)]
  ...
  return (
    <>
      ...
      {modal ?
        <Modal>
          <h1 className="text-2xl font-bold dark:text-white mb-3">Напиши, якою ти бачиш Україну майбутнього!</h1>
          <FormWish
            setModal={setModal}
            setIsOpen={setIsOpen}
